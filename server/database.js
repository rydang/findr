    ...
    db.query('SELECT * FROM friends')
      .then(({ rows }) => {
        if (!rows.length) {
          ...
          let insertQuery =
            'INSERT INTO friends(first_name, last_name, phone_number, username, password) VALUES';
          ...
          return db.query(insertQuery, users);
        }
        return null;
      })
...
router.get('/filterfriends', (req, res) => {
  const { user: username } = req.session;
  db.query(
    'SELECT interests.interest FROM friends, interests WHERE friends.username=$1 AND interests.friend_id=friends.id',
    [username],
  )
    .then(result => {
      const interests = [];
      ...
      const selectQuery = `SELECT * FROM friends WHERE id IN (SELECT friend_id FROM interests WHERE interest IN (${interestsString.toString()}))`;
...
router.post('/friends', (req, res) => {
  const { first_name, last_name, phone_number, username, password, interest } = req.body;
  const parameters = [first_name.trim(), last_name.trim(), phone_number, username, password];
  // parameters[2].replace(/\D/g, '');

  db.query(`SELECT username FROM "friends" WHERE username='${username}'`).then(users => {
    if (users.rows.length) return res.status(400).send('user already exists');
    return db
      .query(
        'INSERT INTO "friends" (first_name, last_name, phone_number, username, password) VALUES ($1,$2,$3,$4,$5) RETURNING id',
        parameters,
      )
      .then(friends => {
        ...
        return db
          .query(insertQuery, interestParameters)
          .then(() => {
            res
  ...
  db.query(`SELECT password FROM friends WHERE username='${username}'`)
    .then(users => {
      ...
      if (users.rows[0].password !== password) {
        return res.status(401).redirect('/error?error=wrongpass');
      }
